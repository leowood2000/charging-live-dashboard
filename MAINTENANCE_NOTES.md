...
- 不要把 `set chg current` 本身当作会话起点；它只有在当前有线/无线边界已确认后才归属该会话。

最后更新：2026-08-11，目标发布版本：v0.11.20。
...
- `last_voters` 按 topic 增量合并；日志窗口没有某个 topic 时保留上一份当前会话数据。
...
- 只展示最新一个会话，每个会话最多 40 个事件；连续设置电流会合并显示。
- 无线会话由 `power_good_on/off` 定界；有线会话由 `USB ONLINE=1/0` 定界，来源切换时封口旧会话并建立新会话。
