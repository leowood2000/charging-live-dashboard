...
- 无线电池侧上限按当前功率路径选择：CP 生效 → quick wireless `cur_max:[Final]`（算法决策，带年龄/历史值标注）+ 无线热控上限（`buck_charge_curr` 中启用的 `wireless_thermal_*` 票）；Buck 生效 → `buck_charge_curr` effective；路径未确认 → 显示“待确认”，不用 Buck FCC 冒充当前上限；`buck_charge_curr` 在 CP 下标注“Buck 路径 FCC”
